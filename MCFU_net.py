...
# 通道注意力模块
class CA(nn.Module):
    def __init__(self, channel, reduction=16, device=None):
    ...
    def forward(self, x):
        ...
        avg_out = self.fc(y_avg).view(b, c, 1, 1)
        max_out = self.fc(y_max).view(b, c, 1, 1)
...
class MBWC(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(MBWC, self).__init__()
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        # 分支1
        self.branch1 = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 1, padding=0, bias=False),
            WTConv2d(out_channels, out_channels, kernel_size=3, stride=1, bias=False, wt_levels=1, wt_type='db1'),
            CA(out_channels, device=self.device),
        ...
        self.branch2 = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 1, padding=0, bias=False),
            WTConv2d(out_channels, out_channels, kernel_size=5, stride=1, bias=False, wt_levels=1, wt_type='db1'),
            SA(device=self.device),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True)
        )
        ...
        self.branch3 = nn.Sequential(
            # TWConv(in_channels, out_channels, padding=1, dilation=1),
            nn.Conv2d(in_channels, out_channels, 1, padding=0, bias=False),
            WTConv2d(out_channels, out_channels, kernel_size=3, stride=1, bias=False, wt_levels=1, wt_type='db1'),
            SA(device=self.device),
            nn.BatchNorm2d(out_channels),
        ...
        # 合并处理
        self.post_conv = nn.Conv2d(3 * out_channels, out_channels, 1, bias=False)
    ...
    def forward(self, x):
        ...
        shuffled = channel_shuffle(concat, groups=3)
        adjusted = self.post_conv(shuffled)

        # 全局空间注意力
        sa = SA(device=self.device)(adjusted)
        sa = self.final_bn(sa)
        sa = self.final_relu(sa)
        out = self.bn(sa + f4)
...
class DoubleConv1(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.conv = nn.Sequential(
            MBWC(in_channels=in_ch, out_channels=out_ch),
            MBWC(in_channels=out_ch, out_channels=out_ch)
        )
